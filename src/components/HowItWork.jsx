      ...
      <div className="container">
        <div className={"w-full lg:w-2/3 md:mx-auto text-center mb-8"}>
          ...
          <p className={"mt-4 text-zinc-50 leading-6"}>
            Booking our cleaning services is easy and stress-free. Simply follow
            these steps for a seamless experience.
          </p>
        </div>
        <div
          className={"grid grid-cols-1 space-y-6 md:grid-cols-3 items-center"}
        >
          {process.map((item) => (
            <div
              key={item.id}
              className={`flex flex-col gap-2 items-center ${
                item.id % 2 === 0 ? "" : "hover:scale-110"
              } transition-all duration-300 ease-in-out`}
            >
              <div
                className={`${item.id % 2 === 0 ? "w-60" : "w-48"}  relative`}
              >
                <img
                  src={item.image}
                  alt=""
                  className={`w-full rounded-full border-8 ${
                    item.id % 2 === 0 ? "border-pink-200" : "border-steel-blue"
                  }`}
                />
                <span
                  className={`${
                    item.id % 2 === 0
                      ? "w-12 h-12 top-2 bg-steel-blue text-pink-100"
                      : "w-10 h-10 bottom-0 bg-pink-100 text-steel-blue"
                  }  flex justify-center items-center rounded-full font-bold text-lg absolute right-4`}
                >
                  {item.id}
                </span>
              </div>
              <h3
                className={`text-xl ${
                  item.id % 2 === 0 ? "text-steel-blue" : "text-white"
